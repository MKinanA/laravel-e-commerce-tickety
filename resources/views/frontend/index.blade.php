...
<section id="eventSection" class="container relative max-w-screen-xl py-10">
    ...
    <div class="flex justify-between items-center gap-4 mb-[50px]">
        ...
        <a href="{{ request()->fullUrlWithQuery(['all_events' => true]) }}" class="btn-primary">
          View All
        </a>
        @endif
    </div>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
        ...
        <x-frontend.card-event :cover="$event->thumbnail" :title="$event->name" :category="$event->category->name" :date="$event->start_time" :price="$event->start_from" :isPopular="$event->is_popular" :description="$event->headline" :route="route('detail', $event->slug)"/>
        @endforeach
    </div>
</section>
...
<section id="categoriesSection" class="relative pb-[100px] overflow-hidden">
    <div class="container relative max-w-screen-xl py-10">
        ...
        <div class="flex justify-between items-center gap-4 mb-[50px]">
            ...
            <a href="{{ request()->fullUrlWithQuery(['all_categories' => true]) }}" class="btn-primary">
                View All
            </a>
            @endif
        </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-[30px] relative">
            ...
            <x-frontend.card-category :icon="$category->thumbnail ?? asset('assets/svgs/ic-movie.svg')" :name="$category->name" :totalEvents="$category->events_count" :route="request()->fullUrlWithQuery(['category' => $category->id])"/>
            @endforeach
        </div>
    </div>
